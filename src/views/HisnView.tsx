...
export const HisnView: React.FC = () => {
  ...
  const handleTap = (id: number) => {
    setAzkar(prev => prev.map(z => z.id === id && z.current > 0 ? { ...z, current: z.current - 1 } : z));
  };
  ...
  const handleSetTimer = async () => {
    ...
    if (success) {
      setNotificationSuccess(`تم ضبط المنبه بنجاح في ${timerHour}:${timerMinute}`);
      setTimeout(() => {
        ...
        {showTimerModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
              ...
              <div className="flex items-center justify-center gap-4 text-3xl font-bold text-slate-800 dark:text-slate-100 my-8" dir="ltr">
                <select 
                  value={timerHour} 
