...
export const QuranView: React.FC = () => {
  ...
  const [selectedSurah, setSelectedSurah] = useState<SurahReference | null>(
    null,
  );
  ...
  return (
    <div className="relative h-full bg-slate-50 dark:bg-slate-950">
      <div
        className="absolute inset-0 bg-cover bg-center bg-fixed opacity-5 dark:opacity-10 pointer-events-none z-0"
        style={{ backgroundImage: `url(${quranImg})` }}
      ></div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    <h3 className="font-bold font-serif text-lg text-slate-800 dark:text-slate-100">
                      {surah.name}
                    </h3>
...
const SurahReader: React.FC<{ surah: SurahReference; onBack: () => void }> = ({
  surah,
  onBack,
}) => {
      ...
      <div className="relative z-10 flex-1 overflow-y-auto p-4 pb-24 space-y-6">
        {loading ? (
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <div
              className="text-justify leading-[2.5]"
              style={{ direction: "rtl" }}
            >
              {quranData.map((ayah, idx) => (
                <React.Fragment key={ayah.numberInSurah}>
                  <span
                    className={cn(
                      ...
                      showTafsir[ayah.numberInSurah]
                        ...
                        : "text-slate-800 dark:text-slate-100 hover:text-emerald-600 dark:hover:text-emerald-500",
                    )}
                  ...
                  {showTafsir[ayah.numberInSurah] && (
                    ...
                    >
                      <div className="font-bold text-emerald-800 dark:text-emerald-400 mb-2 font-serif text-lg">
                        التفسير الميسر:
                      </div>
                      {tafsirData[idx]?.text}
                    </motion.div>
