...
import { SearchableBookViewer } from '../components/SearchableBookViewer';
...
export const LibraryView: React.FC = () => {
  ...
  if (selectedBook) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col bg-slate-50 dark:bg-slate-950">
        <div className="relative z-20 sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 flex items-center gap-4 border-b border-slate-200 dark:border-slate-800 shadow-sm">
          <button 
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500 font-kufi">{selectedBook.author}</p>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto p-4 md:p-6 scroll-smooth">
          {selectedBook.type === 'json' ? (
            <SearchableBookViewer url={selectedBook.sourceUrl} isBukhari={selectedBook.id === 'bukhari'} />
          ) : (
            <div className="font-kufi">
              <div 
                className="leading-loose text-lg text-slate-800 dark:text-slate-200 pb-10 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm"
  ...
  return (
    <div className="p-4 space-y-6 pb-24 h-full overflow-y-auto font-kufi">
